...
describe('editing a due date', () => {
  ...
  it('opens an editor pre-filled with the current due date', () => {
    ...
    expect(editor.input.value).toBe('2026-01-15T09:00');
  });
  ...
  it('updates the displayed date on save', () => {
    ...
    editor.input.value = '2026-03-20T09:00';
    click(editor.save);

    expect(ui.store.getTasks()[0]!.dueDate).toBe('2026-03-20T09:00');
    const time = within<HTMLTimeElement>(ui.row(id), '.task__due');
    expect(time.dateTime).toBe('2026-03-20T09:00');
    expect(time.textContent).toBe(formatDueDate('2026-03-20T09:00'));
  });
  ...
  it('shows the time as well as the date', () => {
    ...
    const editor = openEditor(ui.row(id));
    editor.input.value = '2026-01-15T16:45';
    click(editor.save);

    const time = within<HTMLTimeElement>(ui.row(id), '.task__due');
    expect(time.dateTime).toBe('2026-01-15T16:45');
    expect(time.textContent).toBe(formatDueDate('2026-01-15T16:45'));
    expect(time.textContent).not.toBe(formatDueDate('2026-01-15T09:00'));
    expect(time.textContent).toContain('45');
  });

  it.each([
    ['an empty value', ''],
    ['an impossible calendar date', '2026-02-31T09:00'],
    ...
    expect(ui.store.getTasks()[0]!.dueDate).toBe('2026-01-15T09:00');
    expect(ui.persisted()[0]!.dueDate).toBe('2026-01-15T09:00');
  ...
  it('discards an abandoned edit on cancel', () => {
    ...
    const editor = openEditor(ui.row(id));
    editor.input.value = '2026-09-09T09:00';
    click(editor.cancel);

    expect(ui.store.getTasks()[0]!.dueDate).toBe('2026-01-15T09:00');
    // Re-opening shows the stored value, not the abandoned edit.
    expect(openEditor(ui.row(id)).input.value).toBe('2026-01-15T09:00');
  });
  ...
  it('saves on Enter and closes on Escape', () => {
    ...
    const editor = openEditor(ui.row(id));
    editor.input.value = '2026-04-04T09:00';
    editor.input.dispatchEvent(
      new KeyboardEvent('keydown', { key: 'Enter', bubbles: true, cancelable: true }),
    );
    expect(ui.store.getTasks()[0]!.dueDate).toBe('2026-04-04T09:00');
  ...
  it('keeps the changed due date across a reload', () => {
    ...
    root.querySelector<HTMLInputElement>('.task__due-input')!.value = '2026-07-01T09:00';
