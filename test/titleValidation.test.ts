...
describe('validateNewTask', () => {
  it('accepts a title and a valid ISO due date', () => {
    expect(validateNewTask({ title: 'Buy milk', dueDate: '2026-01-15' })).toEqual({
      valid: true,
      errors: {},
    });
  });
  ...
  it.each(['', '   ', '\t', '\n  \n'])('rejects the blank title %j', (title) => {
    const result = validateNewTask({ title, dueDate: '2026-01-15' });
  ...
  it('rejects an empty or malformed due date', () => {
    ...
    expect(validateNewTask({ title: 'ok', dueDate: '2026-02-31' }).errors.dueDate).toBe(
      INVALID_DATE_MESSAGE,
    );
  });
});
